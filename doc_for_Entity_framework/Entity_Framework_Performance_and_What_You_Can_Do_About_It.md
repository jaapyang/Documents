...
- 在进行**select**操作时，如果我们使用```select * from ...```对每一列进行查询，将不能有效地利用索引来提升查询效率，几乎无法利用索引。一个好的索引策略应该从这几个方面进行考虑与优化：经常被匹配查询的列,在查询中要返回的列,以及创建索引需要的磁盘空间与写入时的附加性能损失之间的平衡.如果你总是查询所有列，这样会变得非常困难。

所以，在查询时我们尽量告诉EF我们需要哪些列，同时可以将这些列形成匿名类型的结果：
